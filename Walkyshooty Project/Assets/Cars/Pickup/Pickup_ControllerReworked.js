// Monitor
private var EngineTorque : float = 0; // Speedometer
private var Gear = 'N';
private var speed = 0;
// var Cam : Camera;
...
function Update () {
	...
	if (EngineRunning == true) {
		...
		Fuel -= (0.001 * FuelConsumption) * Time.deltaTime; // Fuel consumption when engine is idle
		
		if ((audio2.isPlaying==false) && (speed > 0.1)) { //rewrite!!@!@!#!
			audio2.loop = true;
			audio2.volume = 0.4;
		...
		if (audio1.isPlaying == false) { //rewrite!@!@!@
		audio1.loop = true;
		...
		if (speed <= 0.35 * BHP) {
			...
			wheel3.motorTorque -= Gear1 * Time.deltaTime;
		}
		...
		if (speed > 0.4 * BHP) {
			...
			wheel3.motorTorque -= Gear2 * Time.deltaTime;
		}
		
		if (speed > 0.6 * BHP) {
			Gear = '3';
			audio2.pitch = 0.01 * speed;
			wheel3.motorTorque -= Gear3 * Time.deltaTime;
		}
		...
		if (speed > 0.95 * BHP) {
			...
			wheel3.motorTorque -= Gear4 * Time.deltaTime;
		}
	}
	...
	if (Input.GetKeyDown (KeyCode.E)) { // Ignition switch
	...
	if (Input.GetKeyDown (KeyCode.T)) { // Add fuel
		Fuel += 0.5;
	}
	
	
	
	if ((Input.GetKey(KeyCode.W)) && (EngineRunning == true)) {
		
		if (EngineTorque > 0) {
		
			Fuel -= (EngineTorque * 0.001) * 0.001 * FuelConsumption; // Fuel consumption according to current engine power;
			burn = 'NOT ECONOMY';
			
		}
				
		EngineTorque += BHP * Time.deltaTime; // How quick engine is producing torque
		
		if (EngineTorque > BHP) { // EngineTorque capped to BHP
			EngineTorque = BHP;
		}
	}
	else if (EngineTorque > 0) { // When no key is pressed
		EngineTorque -= BHP * Time.deltaTime;
		Fume1.minEmission = 1;
		Fume1.maxEmission = 5;
		Fume2.minEmission = 1;
		Fume2.maxEmission = 5;
	...
	if ((Input.GetKey(KeyCode.S)) && (EngineRunning == true)) {
		var rspeed = -speed;
		if (speed > 0) { // Braking
			light3.intensity = 1; // Brake lights
			light4.intensity = 1;
			...
			if (EngineTorque < 0) {
				Fuel -= (-EngineTorque * 0.001) * 0.001 * FuelConsumption; // Fuel consumption according to current engine torque;
				burn = 'REVERSE';
			}
			...
			/*if (EngineTorque < -(0.50 * BHP)) {
				EngineTorque = -(0.50 * BHP);
			}*/
		}		
	}
	...
	wheel3.motorTorque = EngineTorque * 5; //Redirecting engine power to RWD
	wheel4.motorTorque = EngineTorque * 5; //Redirecting engine power to RWD
...
function OnGUI () {
	GUI.TextArea (Rect (5, 5, 300, 500),'');
	GUI.Label (Rect (10, 30, 200, 30), "Engine Torque: " + EngineTorque);	
	...
	GUI.Label (Rect (10, 290, 600, 30), 'wheel3 trq' + wheel3.motorTorque);
	GUI.Label (Rect (10, 310, 600, 30), 'wheel4 trq' + wheel4.motorTorque);
	GUI.Label (Rect (10, 330, 600, 30), 'wheel1 rpm' + wheel1.rpm);
	GUI.Label (Rect (10, 350, 600, 30), 'wheel2 rpm' + wheel2.rpm);
	GUI.Label (Rect (10, 370, 600, 30), 'wheel3 rpm' + wheel3.rpm);
	GUI.Label (Rect (10, 390, 600, 30), 'wheel4 rpm' + wheel4.rpm);
	GUI.Label (Rect (10, 410, 400, 30), wheel1.steerAngle.ToString());
}